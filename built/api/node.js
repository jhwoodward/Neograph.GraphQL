...
module.exports = function (config) {
    ...
    function updateProperties(n) {
        ...
        return cypher.executeQuery(q, "row", { "id": n.id, "props": that.trimForSave(n) }).then(parseNodeData);
    }
    ...
    function updateLabels(n) {
        ...
        var statements = [];
        //check passed in node against saved node for differences
        return that.get(n).then(function (existing) {
    ...
    var that = {
        ...
        //Get node by (internal ID) or label
        //Add relationships
        getWithRels: function getWithRels(id) {

            var parsed = utils.parseIdOrLabel(id);
            console.log(parsed);
            if (parsed.id) {
                return getNodeById(parsed.id).then(addRelationships);
            }

            if (parsed.label) {
                return getNodeByLabel(parsed.label).then(addRelationships);
            }
        },

        getRelatedItems: function getRelatedItems(obj, reltype, reltypes, classDefs) {
            ...
            let q;
            if (obj.class) {
                //not sure why but no class is being set for images
                q = " match (n:" + obj.class + ")";
            } else {
                q = " match (n)";
            }
            ...
            return cypher.executeQuery(q).then(function (data) {
                return data.map(function (d) {
                    ...
                    if (nolazy) {
                        let ids = d.row[3].map(function (e) {
                            return { id: e };
                        });
                        let props = d.row[2].map(function (e) {
        ...
        getForGraphQL: function getForGraphQL(id, c) {
        ...
        trimForSave: function trimForSave(n) {

            var props = {};

            for (var key in n) {
                if (n[key] !== null && n[key] !== undefined && n[key] !== "" && key !== "labels" && key !== "labelled" && key != "relationships" && key != "image" && key !== "id" && key !== "temp" && key !== "schema" && key !== "web") //web links ?? not implemented yet
                    {
                        props[key] = n[key];
                    }
            }
        ...
        create: function create(n, user) {
            ...
            return cypher.executeQuery(q, "row", { "props": that.trimForSave(n) }).then(function (result) {
        ...
        update: function update(n, user) {
            ...
            return updateLabels(n).then(function () {
        ...
        getSchema: function getSchema(id) {
            return that.getLabels(id).then(function (labels) {
                return _getSchema(labels);
            });
        },
